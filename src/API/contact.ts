...
import { doc, addDoc, collection, onSnapshot, updateDoc, deleteDoc, query, orderBy } from "@firebase/firestore";
...
export const editContactById = (userId: string, profileId: string, contactId: string, updatedData: Partial<ContactType>): Promise<APIResponse<void>> => {
  return new Promise<APIResponse<void>>((resolve, reject) => {
    const contactDocRef = doc(firestore, `users/${userId}/profiles/${profileId}/contacts/${contactId}`);
    
    // Update the document
    updateDoc(contactDocRef, updatedData).catch(error => {
      // Directly reject if there's an error in the write attempt
      reject({ success: false, error: (error as Error).message });
    });
    ...
    const unsubscribe = onSnapshot(contactDocRef, snapshot => {
      ...
      if (!snapshot.metadata.hasPendingWrites) {
        if (snapshot.exists()) {
          ...
          reject({ success: false, error: 'Contact document does not exist post-update.' });
        }
        
        unsubscribe();  // Important: We stop listening after confirming the write.
      }
    }, error => {
      console.error("Snapshot error:", error);
      reject({ success: false, error: error.message });
